...
class XLSRExtractor:
    def __init__(self, device='cpu', batch_size=4, num_workers=1):
        """
        Initialize the XLSR extractor using facebook/wav2vec2-xls-r-1b.
        ...
        print(self.device)
        self.processor = Wav2Vec2FeatureExtractor.from_pretrained("facebook/wav2vec2-xls-r-1b")
        self.model = Wav2Vec2Model.from_pretrained("facebook/wav2vec2-xls-r-1b").to(self.device)
    ...
    def extract_features(self, audio_path):
        """
        Extract features from a single audio file using facebook/wav2vec2-xls-r-1b.
        
        Returns:
            avg_embeddings (np.array): Average embedding over the time dimension.
        """
        waveform, fs = self.preprocess_audio(audio_path)
        if waveform is None:
            return None
        # Convert the waveform to a 1D numpy array and add a batch dimension.
