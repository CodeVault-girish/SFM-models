...
# Import LanguageBind and its helper functions.
# (Make sure LanguageBind is available in your environment.)
...
class LanguageBindExtractor:
    ...
    def initialize_languagebind(self, cache_dir):
        """
        Initialize the LanguageBind model, tokenizer, and audio transformation.
        """
        clip_type = {
            'audio': 'LanguageBind_Audio_FT',  # Use 'LanguageBind_Audio' if you are not using the fine-tuned model.
        }
        ...
        # Load the pre-trained tokenizer from a checkpoint.
        pretrained_ckpt = 'lb203/LanguageBind_Audio'
        logging.info(f"Loading tokenizer from checkpoint: {pretrained_ckpt}")
        tokenizer = LanguageBindAudioTokenizer.from_pretrained(pretrained_ckpt, cache_dir=f'{cache_dir}/tokenizer_cache_dir')
    ...
    def extract_embedding(self, audio_path):
        ...
        :return: A NumPy array of averaged embeddings or None if extraction fails.
        """
        audio_files = [audio_path]  # LanguageBind expects a list of file paths.
        try:
    ...
    def extract_folder(self, folder_path, output_file):
        """
        Process all audio files in the folder and save the extracted features to a CSV file.
        ...
        try:
            audio_files = [f for f in os.listdir(folder_path) if f.lower().endswith(supported_extensions)]
        except Exception as e:
